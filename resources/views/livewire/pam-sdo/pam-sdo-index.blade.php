    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500">Pengamanan Sumber Daya Organisasi (Personil, Materiil, Informasi).</p>
            </div>
        ...
        <div class="bg-white p-4 rounded-lg shadow mb-6 border border-gray-100">
            <input wire:model.live="search" type="text" placeholder="Cari kegiatan, lokasi, atau pelaksana..." class="w-full border-gray-300 rounded-md shadow-sm focus:ring-gray-500 focus:border-gray-500">
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-slate-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Tanggal & Kegiatan</th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Kategori & Pelaksana</th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Lokasi & Status</th>
                            <th class="px-6 py-3 text-center text-xs font-bold text-gray-500 uppercase tracking-wider">Verifikasi</th>
                            <th class="px-6 py-3 text-center text-xs font-bold text-gray-500 uppercase tracking-wider">Dokumentasi</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-[10px] leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">
                                    {{ $item->kategori_pam }}
                                </span>
                                <div class="text-xs text-gray-500 mt-1">Oleh: {{ $item->pelaksana }}</div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                ...
                                <img class="h-10 w-10 rounded object-cover border border-gray-300 shadow-sm mx-auto" src="{{ asset('storage/' . $item->foto_dokumentasi) }}">
                                @else
                                <span class="text-xs text-gray-400">-</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="{{ route('reports.pam-sdo.satuan', $item->id) }}" target="_blank" class="text-gray-500 hover:text-gray-900 mr-3" title="Cetak Data">
                                ...
                                <button wire:click="delete({{ $item->id }})" wire:confirm="Hapus data ini?" class="text-red-600 hover:text-red-900 font-bold text-xs">Hapus</button>
                        ...
                        @empty
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-center text-gray-500 italic font-medium">Belum ada laporan PAM SDO.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            <div class="px-6 py-4 border-t border-gray-200">{{ $pam_sdos->links() }}</div>
        </div>
    </div>
            ...
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6">
                    <h3 class="text-lg font-bold text-gray-900 mb-4 border-b pb-2">
                        {{ $is_edit ? 'Update Laporan PAM SDO' : 'Buat Laporan PAM SDO Baru' }}
                    </h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" wire:model="nama_kegiatan" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-gray-500 focus:border-gray-500 sm:text-sm">
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                ...
                                <div>
                                    ...
                                    <select wire:model="kategori_pam" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm sm:text-sm">
                                        ...
                                        <option value="Pengamanan Materiil">Pengamanan Materiil</option>
                                        <option value="Pengamanan Informasi">Pengamanan Informasi</option>
                                        <option value="Pengamanan Kegiatan">Pengamanan Kegiatan</option>
                                    </select>
                                </div>
                            </div>
                            <div>
                        ...
                        <div class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Pelaksana Tugas</label>
                                <input type="text" wire:model="pelaksana" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm sm:text-sm" placeholder="Nama Petugas...">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Uraian / Keterangan</label>
                            ...
                            <div class="border rounded-lg p-3 bg-gray-50 border-dashed border-gray-300">
                                ...
                                <div class="flex items-center space-x-4">
                                    @if ($foto)
                                    <img src="{{ $foto->temporaryUrl() }}" class="h-16 w-16 object-cover rounded border-2 border-gray-500 shadow-sm">
                                    @elseif ($old_foto)
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse border-t">
                    <button type="button" wire:click="store" wire:loading.attr="disabled" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-gray-800 text-base font-medium text-white hover:bg-gray-900 sm:ml-3 sm:w-auto sm:text-sm disabled:opacity-50 transition">
